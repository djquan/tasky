...
- **Token Server**: <http://localhost:3001/token>
- **Y-Sweet**: ws://localhost:8080

### Configure Tasky

1. Open <http://localhost:80>
2. Settings → Enable sync
3. Enter: `http://localhost:3001/token`
4. Save
...
All services communicate via the `tasky-network` Docker network. The token server replaces internal Docker hostnames with client-accessible URLs (`localhost:8080`) when returning tokens to browsers.
